...
parfor ii=1:numel(DATA_set)
    ...
    m_fluid                     = ( -2*mSOL_f/numel(L_bed_after_nstages) /(L-L_bed_after_nstages(1)) * ( L_nstages(1:end-nstagesbefore(end))-(L-L_bed_after_nstages(1)) ) );                   % Lienarly distirubuted mass of solute in fluid phase, which goes is zero at the outlet
...
for ii=1:numel(DATA_set)
    ...
    m_fluid_opt             = ( -2*mSOL_f_opt/numel(L_bed_after_nstages) /(L-L_bed_after_nstages(1)) * ( L_nstages(1:end-nstagesbefore(end))-(L-L_bed_after_nstages(1)) ) );                   % Lienarly distirubuted mass of solute in fluid phase, which goes is zero at the outlet
