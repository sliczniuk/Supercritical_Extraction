...
Parameters_sym(47)      = ratio(2);
                          ...
                          OPT_solver.subject_to( 0 < DI_correlation(750,ratio) );
...
%OPT_solver.subject_to( K(2+Nk) > K(2) > K(2+3*Nk) > K(2+2*Nk) );   
%OPT_solver.subject_to( K(2+Nk) < K(2) < K(2+3*Nk) < K(2+2*Nk) ); 
...
NAME = {'$k_m[-]$', '$D_e^M[m^2/s] \times 1e-6$', 'sat', '$\tau[-]$', '$\sigma[-]$'};
name = {'km', 'De', 'sat', 'tau', 'sigma'};
...
for ii=1:numel(DATA_set)
    ...
    % Initial conditions
        ...
        hold off
        cb.Label.String = ['$\frac{d c_s}{d',name{ii},'}$']; cb.Label.Interpreter = 'latex'; cb.Label.FontSize = 14;
        ...
        yline([L_nstages(nstagesbed(end)) L_nstages(nstagesbed(1))],'w--',{'end of bed','beginning of bed'}, 'Interpreter', 'latex')
        hold off
        cb.Label.String = ['$\frac{d h}{d',name{ii},'}$']; cb.Label.Interpreter = 'latex'; cb.Label.FontSize = 14;
        ...
        yline([L_nstages(nstagesbed(end)) L_nstages(nstagesbed(1))],'w--',{'end of bed','beginning of bed'}, 'Interpreter', 'latex')
        hold off
        cb.Label.String = ['$\frac{d c_f}{d',name{ii},'}$']; cb.Label.Interpreter = 'latex'; cb.Label.FontSize = 14;
        xlabel('Time [min]'); ylabel('Length [m]')
        ...
        xlabel('Time [min]'); ylabel(['$\frac{d y}{d',name{ii},'}$'])
        hold off
        colormap jet

        set(gcf,'PaperOrientation','landscape'); print(figure(1),[Title{ii},'.pdf'],'-dpdf','-bestfit')
        close all; 
