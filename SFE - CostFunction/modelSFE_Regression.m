function xdot = modelSFE_Regression(x, u, k, parameters)
    % (t, x, u, parameters)
    ...
    P             =     linspace(P_u,1*P_u,nstages_index);
    ...
    DIFFUSION      =    Dx * ones(nstages_index,1);

    %DIofT         = Di_of_T(RHO, parameters);
    DIofT         =     Di*ones(nstages_index,1);

    %KM           = km_of_T(RHO, parameters);
    %KM           = km(x(2*nstages_index+1:3*nstages_index) ,P_u);
    KM            =     km*ones(nstages_index,1);

    CP            =     SpecificHeatComp(x(2*nstages_index+1:3*nstages_index), P, Z,RHO, parameters);
    CPRHOCP       =     cpRHOcp_Comp(x(2*nstages_index+1:3*nstages_index),P,Z,RHO,CP,parameters);
    KRHOCP        =     kRHOcp_Comp(x(2*nstages_index+1:3*nstages_index),P,Z,RHO,CP,parameters);
