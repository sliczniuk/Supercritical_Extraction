...
DATA_K_OUT=nan(4,4);                                           % Store Parameters obatined from all fits (par num x num exper)
...
parfor ii=1:numel(DATA_set)
    ...
    J       = (data-Yield_estimate_diff ) * diag(1:1:numel(data)) * (data-Yield_estimate_diff )';
    %J = sum(abs(data-Yield_estimate))/numel(data);

    % MLE: Normal
    %J_L     = -numel(data)/2 * log(2*pi) - numel(data) * log(sigma) - J/(2*sigma^2);
    ...
    p.pushNote('ujBXuXMcOOasjC3NU4Fk5c','Matlab',['This work is finished', mat2str([KOUT])]);
