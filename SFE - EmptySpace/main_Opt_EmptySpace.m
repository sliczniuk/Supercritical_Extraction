...
r                    = 0.075;                                         % Volume of the extractor  [m3]
L                    = 0.2;                                          % Length of the extractor [m]
%--------------------------------------------------------------------
L_bed                = 0.1;                                          % Length of the fixed bed [m]
V_bed                = L_bed*pi*r^2;                                      % Volume of the fixed bed [m3]
%--------------------------------------------------------------------
epsi_alpha           = floor(nstages*(L_bed / L) );
epsi                 = 2/3;                                          % Porosity [-]

C0solid              = m_ref *1e-3 / (V_bed*(1-epsi ));              % Solid phase kg / m^3
...
%% Set Integrator
f                    = @(x, u) modelSFE(x, u, nstages, L, L_bed);
...
for i=1:numel(DATA)
     ...
     x0         = [C0fluid*ones(nstages,1);
                  ...
                  %[C0solid * ones(epsi_alpha,1); zeros(nstages-epsi_alpha,1) ];
