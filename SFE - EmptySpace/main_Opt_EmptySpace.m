...
Nk                      = numel(which_k)+2;                                      % model parameters + 0*sigma + initial concentration
...
parfor ii=1:numel(DATA_set)
    ...
    mSOL_s                   = k(5);                                          % g of product in biomass
    max_ms                   = k(6);
    mSOL_f                   = max_ms-mSOL_s;                                   % g of biomass in fluid
    
    k_lu                    = [ [0;0.01;0.01;0;0;75] , [1;10;1;10;max_ms;100] ];
    % Constraints
    for nk=1:Nk
    ...
    Parameters([1,3:9])     = [nstages, r, epsi, dp, L, rho_s, km, mi];
...
for ii=1:numel(DATA_set)    
    ...
    %% Set parameters
    mSOL_s                   = DATA_K_OUT(5,ii);                              % g of product in biomass
    max_ms                   = DATA_K_OUT(6,ii);
    mSOL_f                   = max_ms-mSOL_s;                                   % g of biomass in fluid
...
%% Collect parameters and fit a line and quadratic function as a function of \rho
