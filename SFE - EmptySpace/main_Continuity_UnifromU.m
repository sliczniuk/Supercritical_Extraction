...
%feedTemp( round(numel(Time)/10) : round(numel(Time)/2) )   = T0homog - 10;
%feedTemp( round(numel(Time)/2)  : end )   = T0homog ;

feedPress  = feedPress * ones(1,N_Time) + 0 ;  % Bars
%feedPress(round(numel(Time)/4):round(numel(Time)/2))   = feedPress(1) - 10;
