...
%%
k1 = linspace(0.01,1,25);
k2 = linspace(0.01,1,25);
k3 = linspace(0.01,1,25);

X=cell(numel(k1),numel(k2),numel(k3));
...
p.pushNote([],'Matlab:','The computations are started');

tic

parfor ix=1:prod(iterations)
    [a,b,c]=ind2sub(iterations,ix);
    X{ix}={k1(a),k2(b),k3(c),full(fJ([k1(a),k2(b),k3(c)]))};

    ppm.increment();
end
toc

p.pushNote([],'Matlab:','The computations are done');
...
%data_norm = [normalize(data(:,1:3), 'range'), data(:,4)];

%%

labels = {'$k_m$' '$D_i$' '$D_x$' 'fJ'};
...
%set(gcf,'PaperOrientation','landscape'); print(figure(1),['Sensitivity_',Var_name,'_Profiles.pdf'],'-dpdf','-bestfit'); close all
...
subplot(2, 2, 1)
ternplot(A, B, C, '.', 'majors', 25); 
ternlabel(labels{1:3}); 
