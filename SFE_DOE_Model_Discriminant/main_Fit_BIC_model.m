...
Parameters_table        = readtable('Parameters_BIC.csv') ;                     % Table with prameters
...
%%
which_k                 = [    44,   45,   46,   47             ];              % Select which parameters are used for fitting
k0                      = [  1e-2, 1e-2, 1e-4, 1e-4,   0.5, 0.05];              % Give inital value for each parameter
k_lu                    = [ [   0;    0;    0;    0;   0  ; 0   ], ...
                          [   inf;  inf;  inf;  inf;   1  ; inf ] ];
Nk                      = numel(which_k)+2;                                   % Parameters within the model + psi, sigma
...
%% Number of variables
Nx                      = 4 * nstages+2;                                    % 3*Nstages(C_f, C_s, H) + P(t) + yield
...
%% Set Integrator
f                       = @(x, u) modelSFE_BIC(x, u, bed_mask, timeStep_in_sec);
xdot                    = modelSFE_BIC(x, u, bed_mask, timeStep_in_sec);
...
C0solid_Broken          = C0solid * (1-psi)                ;                % Solid phase kg / m^3
C0solid_Intact          = C0solid * psi                    ;                % Solid phase kg / m^3
...
% Initial conditions
x0                      = [ C0fluid'                         ;
                            C0solid_Broken  * bed_mask       ;
                            C0solid_Intact  * bed_mask       ;
...
for i=1:numel(which_k)
    Parameters{which_k(i)}  = KOUT(i);
end

C0solid                 = mSOL_s * 1e-3 / ( V_bed * epsi)  ;                % Solid phase kg / m^3
C0solid_Broken          = C0solid * (1-KOUT(5))            ;                % Solid phase kg / m^3
C0solid_Intact          = C0solid * KOUT(5)                ;                % Solid phase kg / m^3
Parameters{2}           = C0solid;
