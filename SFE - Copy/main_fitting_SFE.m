...
RHO = [];
k0 = [1;1;1;1];
%k0 = [1.970757180127422, 0.008050437716460, 0.500001015151448, 0.010008918267933, 0.010194595171203, 0.010567948706647, 0.009000685797227]'*1e2;
KOUT = k0;
...
for l = 1:numel(DATA)
    ...
    data2 = diff(data);
    %data = yy;
    %data = data(1:N_Sample:end);
    %fprintf('\nInitial simulation is finished\n')

    %xx(end,end)
    %%
    %y = modelSFE_out2(xx, uu, 0, parameters, timeStep_in_sec);
    %hold on; plot(yy,'o'); plot(y); hold off

    %%
    OCP = struct('Nx', Nx, 'Nu', Nu, 'Nk', Nk, 'Ny', Ny, 'N', length(Time_in_sec), ...
        'x_lu', [],  ...                % 0*ones(Nx,1) inf*ones(Nx,1)
        'k_lu', [0*ones(Nk,1) inf*ones(Nk,1)],  ...                % 0*ones(Nk,1) [1.5;inf;inf]
        'x_eq', [], ...
        'k_eq', [], ...
        'N_Sample', N_Sample,...
        'N_Delay', N_Delay,...
...
%     fprintf('-------------------------------------------------\n')
    ...
    caption1 = sprintf('T = %g [K], P = %g [bar]\n km = %g, Di = %g, Dx = %g, \\sigma = %g', T0homog, feedPress(1), kout');
    title(caption1, 'Interpreter','tex');

    xlabel('Time [min]')
    ylabel('Yield [%]')

    Name = strcat(string(l),'_General_Fitting_T_',string(T0homog-273.15),'_P_',string(feedPress(1)),'.pdf');

    print(gcf, Name,'-dpdf','-r700');

end
%%
%figure(); hold on; plot(Time(N_Sample+1:N_Sample:end), diff(data), 'o','Color','k','LineWidth',1); plot(Time(N_Sample+1:N_Sample:end),diff(yy_out_D(1:N_Sample:end))); hold off

%close all

%% save data
%save SFE_fitting.mat