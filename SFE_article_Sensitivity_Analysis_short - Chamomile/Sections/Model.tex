...
\begin{document}
	...
	%The comprehensive derivation of the mass balance equation for the fluid phase is detailed in Appendix \ref{CH: Gouverning equations}. 
			...
			{\footnotesize
				\begin{equation} \label{Model_heat}
					\begin{split}
						...
						\underbrace{ -\cfrac{F {\color{black}C_p}}{{\color{black}A} 	[(1-{\color{black}\epsilon})\rho_f {\color{black}C_p}  + {\color{black} \epsilon \rho_s C_{ps} } ]} \cfrac{\partial T}{\partial z}  }_{\text{Convection}} + \\
						& + \underbrace{ {\color{black}D^T_e} \cfrac{\partial^2 T}{\partial {\color{black}z^2}} }_{\text{Diffusion}}
					\end{split}
			\end{equation} }
			
		where $ {\color{black}D^M_e}(T,P,F)$ is the axial mass diffusion coefficient, ${\color{black}C_p}$ is the fluid's specific heat, ${\color{black}C_{ps}}$ is the specific heat of the solid phase, ${\color{black}D^T_e}(T,P,F)$ is the axial heat diffusion coefficient. 
...
%		\todo[inline]{Draft of new heat equation starts here. TODO: \\ Check if the heat cpacity of solid phase needs to be added \\ Elaborate why pressure is know}
			...
			The heat governing equation describe the evolution of the internal energy in the system and it is given by Equation \ref{EQ: Heat_internal}. The derivation of the heat equation can be found in the work of \citet{Anderson1995}.
			
			{\footnotesize
			\begin{equation} \label{EQ: Heat_internal}
			...
			Following \citet{Elliott2011} or \citet{Gmehling2019}, a real gas internal energy definition can be obtained from the departure functions, defined through Equation \ref{EQ: internal_energy_def}.	The departure function is a mathematical function that characterizes the deviation of a thermodynamic property (enthalpy, entropy, and internal energy) of a real substance from that of an ideal gas at the same temperature and pressure. The departure function is typically defined as the difference between the value of a thermodynamic property for a real fluid and the corresponding value for an ideal gas at the same temperature and pressure. They are typically computed by integrating a function that depends on the equation of state and its derivatives.

			{\footnotesize
				\begin{equation}
					de=C_vdT-\left[P-T\left(\cfrac{\partial P}{\partial T}\right)_{{\color{black}v_m}}\right]d{\color{black}v_m}
					\label{EQ: internal_energy_def}
				\end{equation} }
			
			where $e^{id}$ is the internal energy of perfect gas.
			
			Suppose a gas is considered to be perfectly caloric ($e={\color{black}C_v} T$), then the energy equation can be written explicitly in the form of temperature. The perfectly caloric gas can be seen as the special case of a real gas, where the second term of Equation \ref{EQ: internal_energy_def} goes to zero and the heat capacity ${\color{black}C_v}$ is constant.
...
%			}
			...
			{\scriptsize
				\begin{equation}
					\cfrac{e-e^{id}}{RT} = - \cfrac{{\color{black}A}}{{\color{black}B}\sqrt{8}} \cfrac{{\color{black}\kappa} \sqrt{\color{black}T_r}}{\sqrt{\alpha}} \ln \left[ \cfrac{{\color{black}Z}+\left( 1+\sqrt{2} \right){\color{black}B}}{{\color{black}Z}+\left( 1-\sqrt{2} \right){\color{black}B}} \right]
					\label{EQ: internal_energy_PR}
			\end{equation}
			}
...
%				y \left[ kg \right] &= \int_{t_0}^{t_f} Q \left[ \cfrac{m^3}{s} \right] c_f \biggr\rvert_{z=L} \left[ \cfrac{kg}{m^3} \right] dt \left[ s \right] \\
				y &= \int_{t_0}^{t_f} \cfrac{F}{\rho_f} c_f \biggr\rvert_{z=L} dt \\
%				\cfrac{dy}{dt} \left[ \cfrac{kg}{s} \right] &= Q \left[ \cfrac{m^3}{s} \right] c_f \biggr\rvert_{z=L} \left[ \cfrac{kg}{m^3} \right]  \\
		...
		%The state-space model ${\color{black}\chi}$ after the discretization is represented by $\dot{{\color{black}x}}$.
			
		%In a state-space sense, the state variables of the system are the local concentrations of solute in the fluid and solid phases ($c_f$ and $c_s$, respectively), and the local enthalpy of the pseudo-homogeneous phase (${\color{black}h}$). The controllable input variables are the mass flow rate and temperature of the solvent in the feed and the pressure in the extractor. Additionally, the pressure change is augmented with the state-space and denoted as $P$. 
		%The system state-space is extended by assuming that extraction yield can be modelled as a function of a known initial mass of solute in the solid phase, and it can be measured after the separator (${\color{black}Y}$). The system is controllable by manipulating the flow rate and temperature (enthalpy) of CO$_2$ in the feed and the pressure in the extractor.
