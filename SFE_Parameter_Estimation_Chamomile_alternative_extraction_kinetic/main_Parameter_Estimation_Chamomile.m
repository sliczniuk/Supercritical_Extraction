...
excel_file = 'Chamomile_Di_Gamma_Langmuir.xls';
rng(69)
...
for ii=1:N_trial
    ...
    %set(gcf,'PaperOrientation','landscape'); print(figure(jj),['Fit_Di_Gamma_dataset_',num2str(jj),'_org.pdf'],'-dpdf','-bestfit')
    %close all
...
XX = linspace(KOUT(1)/5,KOUT(1)*5,50);
YY = linspace(KOUT(2)/5,KOUT(2)*5,60);
[X,Y] = meshgrid(XX,YY);
