...
\begin{document}
	...
	\subsubsection{Continuity equation} \label{CH: Continuity}
	 %When the cross-sectional area of the channel $A_f$ is specified as a function of the void fraction $\phi$ (where ${\color{black}\phi}$ is the void fraction of the bed and $A$ is the cross-section of the empty extractor), the continuity equation takes the form: %\todo[]{assign a color of v and u}
	 
	%The obtained above quasi-one-dimensional continuity equation (Equation \ref{EQ: CompressibleEuler_1}) is further modified by specifying a function $A_f = A\phi$ to take into account the change of the cross-section available for the fluid. The Equation \ref{EQ: Continuity_differential} shows the differential form of the continuity equation: 
	...
	{\footnotesize
		\begin{equation} \label{EQ: Continuity_differential}
			\frac{\partial (\rho_f \phi)}{\partial t} + \frac{\partial (\rho_f v A\phi)}{\partial {\color{black}z}} = 0
		\end{equation}
	}
	where ${\color{black} A}$ is the total cross-section of the extractor and $\phi$ describe porosity along the extractor.
	...
	{\footnotesize
		\begin{equation}
			\int \frac{\partial (\rho_f v A \phi )}{\partial {\color{black}z}} dz = F \rightarrow {\color{black}F}=\rho_f v A\phi
		\end{equation}
	}
	
	To simplify the dynamics of the system, it is assumed that ${\color{black}F}=F$ is a control variable and affects the whole system instantaneously. This assumption allows for finding the velocity profile that satisfies mass continuity based on $F$, $\phi$, and $\rho_f$.
	...
	{\footnotesize
		\begin{equation}
			u = v \phi = \cfrac{F}{\rho_f {\color{black} A} }
		\end{equation}
	}
	...
	{\footnotesize
		\begin{equation}
			...
			+ \frac{1}{\phi} \frac{\partial \left( c_f u\right)}{\partial {\color{black}z}}
			= \frac{1-\phi}{\phi} r_e
			+ \frac{1}{\phi} \frac{\partial}{\partial {\color{black}z}} \left( {\color{black}D^M_e} \frac{\partial c_f}{\partial {\color{black}z}} \right)
		\end{equation}
	}
	
	$c_f$ represents the concentration of solute in the fluid phase, , $r_e$ is a mass transfer kinetic term, and ${\color{black}D^M_e}(T(t,z),{\color{black}P}(t),F)$ is the axial diffusion coefficient.
		...
		On the other hand, \citet{Reverchon1996} proposes a driving force given by the difference between ${\color{black}c_s}$ and ${\color{black}c_p^*}$. ${\color{black}c_p^*}$ is determined by an equilibrium relationship with ${\color{black}c_f}$ and the extraction rate is ${\color{black}r_\text{e}}\left({\color{black}c_s} - {\color{black}c^*_p}({\color{black}c_f})\right)$ or more precisely
			...
			{\footnotesize
				\begin{equation} \label{Model_kinetic_basic}
					r_e = \cfrac{{\color{black}D_i}(T(t,z), {\color{black}P}(t), F(t))}{{\color{black} \mu l^2} }\left(c_s - {\color{black}c_P^*}(t,z) \right)
			\end{equation} }
			
		where ${\color{black}\mu}$ is sphericity, ${\color{black}l}$ a characteristic dimension of particles and can be defined as ${\color{black}l} = {\color{black}r}/3$, ${\color{black}r}$ is the mean particle radius, ${\color{black}\rho_s}$ is the solid density, ${\color{black}D_i}(T(t,z))$ corresponds to the overall diffusion coefficient and ${\color{black}c_P^*}(t,z)$ is a concentration at the solid-fluid interface (which according to the internal resistance model is supposed to be at equilibrium with the fluid phase). 
			
		According to \citet{Bulley1984}, a linear equilibrium relationship (equation  \ref{Linear_equilibirum}) can be used to find an equilibrium concentration of the solute in the fluid phase ${\color{black}c_f^*}(t,z)$ is based on the concentration of the solute in the solid phase $c_s$ 
			
			{\footnotesize
				\begin{align} \label{Linear_equilibirum}
					{\color{black}c}(t,z) &= {\color{black}k_p}(T(t,z),{\color{black}P}(t)) \allowbreak {\color{black}q^*}(t,z)
			\end{align} }
			
			The volumetric partition coefficient ${\color{black}k_p}(T(t,z),{\color{black}P}(t))$ behaves as an equilibrium constant between the solute concentration in one phase and the corresponding equilibrium concentration at the solid-fluid interphase. According to \citet{Spiro2007}, the term ${\color{black}k_p}(T(t,z),{\color{black}P}(t))$ can be expressed as the function of mass partition factor ${\color{black}k_m}(T(t,z))$.
			
			{\footnotesize
				\begin{align}
					{\color{black}k_m}(T(t,z)) = \cfrac{{\color{black}k_p}(T(t,z),{\color{black}P}(t)) {\color{black}\rho_s}}{{\color{black}\rho}(T(t,z),{\color{black}P}(t))}
			\end{align} }
			...
			{\footnotesize
				\begin{equation}
					...
					r_e = -\cfrac{{\color{black}D_i}(T(t,z), {\color{black}P}(t), F(t))}{{\color{black} \mu l^2} }\left(c_s - \cfrac{{\color{black}\rho_s}c_f(t,z)}{{\color{black}k_m}(T(t,z))\rho_f}   \right)
			\end{equation} }
			...
			{\footnotesize
				\begin{equation} \label{Model_heat}
					\begin{split}
						...
						\underbrace{ -\cfrac{F {\color{black}C_p}(T(t,z),{\color{black}P}(t))}{{\color{black}A} 	[(1-{\color{black}\epsilon})\rho_f {\color{black}C_p} (T(t,z),{\color{black}P}(t)) + {\color{black} \epsilon \rho_s C_{ps} } ]} \cfrac{\partial T(t,z)}{\partial {\color{black}z}}  }_{\text{Convection}} + \\
...
%		\todo[inline]{Draft of new heat equation starts here. TODO: \\ Check if the heat cpacity of solid phase needs to be added \\ Elaborate why pressure is know}
			...
			\begin{align} \label{EQ: Heat_internal}
				&\cfrac{\partial \left( \rho_f e(t,z) A_f \right) }{\partial t} + \cfrac{\partial \left( \rho_f A_f v e(t,z) \right)}{\partial {\color{black}z}} \nonumber \\
				&= -{\color{black}P}(t)\cfrac{\left( A_f v \right)}{\partial {\color{black}z}} + \cfrac{\partial}{\partial {\color{black}z}} \left( \cfrac{\partial T(t,z)}{\partial {\color{black}z}} \right) 
			\end{align}
			}
...
%			}
			...
			{\footnotesize
				\begin{align} \label{EQ:Enthalpy_equation}
					\cfrac{\partial \left(\rho_f {\color{black}h}(t,z) A_f\right)}{\partial t} &= - \cfrac{\partial \left( \rho_f {\color{black}h}(t,z) A_f v \right)}{\partial {\color{black}z}}  \nonumber \\
					& + \cfrac{\partial \left({\color{black}P}(t) A_f\right)}{\partial t} + \cfrac{\partial}{\partial {\color{black}z}} \left( k \cfrac{\partial T(t,z)}{\partial {\color{black}z}} \right)
				\end{align}
			}
...
%				y(t) \left[ kg \right] &= \int_{t_0}^{t_f} Q(t,z) \left[ \cfrac{m^3}{s} \right] c_f(t,z) \biggr\rvert_{z=L} \left[ \cfrac{kg}{m^3} \right] dt \left[ s \right] \\
				{\color{black}y}(t) &= \int_{t_0}^{t_f} \cfrac{F}{\rho_f} c_f \biggr\rvert_{z=L} dt \\
%				\cfrac{dy}{dt} \left[ \cfrac{kg}{s} \right] &= Q(t,z) \left[ \cfrac{m^3}{s} \right] c_f(t,z) \biggr\rvert_{z=L} \left[ \cfrac{kg}{m^3} \right]  \\
				\cfrac{d{\color{black}y}(t)}{dt} &= \qquad \cfrac{F}{\rho_f} c_f \biggr\rvert_{z=L} 
		...
		\end{comment}
			...
			{\footnotesize
				\begin{align*}
					&{\color{black}c_f}(t = 0, z) = 0 && c_s(t = 0, z) = {\color{black}c_{s0}} && {c_f}(t, z=0) = 0 \\
	...
	\end{comment}
		...
		{\footnotesize
			\begin{align}
				...
				& =
				\begin{bmatrix}
					{\color{black}\bar{\phi_1}} \left( c_f,c_s,{\color{black}h}(t,z); {\color{black}\Theta} \right)\\
					{\color{black}\bar{\phi_2}} \left( c_f,c_s,{\color{black}h}(t,z); {\color{black}\Theta} \right)\\
					{\color{black}\bar{\phi_3}} \left( c_f,c_s,{\color{black}h}(t,z); {\color{black}\Theta} \right) \\
					{\color{black}\bar{\phi_4}} \left( c_f,c_s,{\color{black}h}(t,z); {\color{black}\Theta} \right)\\
					{\color{black}\bar{\phi_5}} \left( c_f,c_s,{\color{black}h}(t,z); {\color{black}\Theta} \right)
